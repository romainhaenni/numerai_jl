...
function fillna(df::DataFrame, value::Float64=0.0)::DataFrame
    ...
    for col in names(df_copy)
        df_copy[!, col] = coalesce.(df_copy[!, col], value)
    end
    return df_copy
end
...
function gaussianize(values::Vector{Float64})::Vector{Float64}
    ...
    lower_bound = 1.0 / (2 * length(values))
    upper_bound = 1.0 - lower_bound
    ranks = clamp.(ranks, lower_bound, upper_bound)
    
    return quantile.(Normal(), ranks)
end
...
function normalize_predictions(predictions::Vector{Float64})::Vector{Float64}
    min_val, max_val = extrema(predictions)
